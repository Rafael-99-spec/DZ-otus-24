...
Например папку /etc/123456789, и 5 файлов внутри папки. 
```
[root@client vagrant]# mkdir /etc/123456789
[root@client vagrant]# > /etc/123456789/1
[root@client vagrant]# > /etc/123456789/2
[root@client vagrant]# > /etc/123456789/3
[root@client vagrant]# > /etc/123456789/5
[root@client vagrant]# > /etc/123456789/4
[root@client vagrant]# ll /etc/123456789
total 0
-rw-r--r--. 1 root root 0 Sep  9 09:13 1
-rw-r--r--. 1 root root 0 Sep  9 09:13 2
-rw-r--r--. 1 root root 0 Sep  9 09:13 3
-rw-r--r--. 1 root root 0 Sep  9 09:13 4
...
[root@client /]# rm -rf /etc/123456789/
[root@client /]# ll /etc/123456789
ls: cannot access /etc/123456789: No such file or directory
[root@client /]#
```
Восстановим копию папки /etc из borgbackup 
```
[root@client /]# borg list 192.168.111.10:/var/backup
Enter passphrase for key ssh://192.168.111.10/var/backup: 
client-2020-09-09-09-20              Wed, 2020-09-09 09:20:01 [3314823ea68730cf3139d4b249dd5435044e64a4d4ad31cc5c18062c7bc8cd6e]
[root@client /]# borg extract --list 192.168.111.10:/var/backup::client-2020-09-09-09-20
Enter passphrase for key ssh://192.168.111.10/var/backup: 
etc
etc/fstab
etc/crypttab
...
etc/systemd/system/vmtoolsd.service.requires/vmtoolsd-init.service
...
etc/systemd/system/multi-user.target.wants/borg-backup.timer
...
etc/systemd/system/sysinit.target.wants/rhel-autorelabel-mark.service
...
etc/xinetd.d
etc/ld.so.conf.d
etc/ld.so.conf.d/kernel-3.10.0-1127.19.1.el7.x86_64.conf
...
etc/selinux/targeted/active/modules/100/boltd
etc/selinux/targeted/active/modules/100/boltd/lang_ext
etc/selinux/targeted/active/modules/100/boltd/cil
etc/selinux/targeted/active/modules/100/boltd/hll
...
etc/selinux/targeted/active/modules/100/kpatch
...
```
И наконец посмотрим появилась ли удаленная папка вместе с файлами, 
```
[root@client /]# ll /etc/123456789/
total 0
-rw-r--r--. 1 root root 0 Sep  9 09:13 1
